...
if (-not $exeAsset) {
    Write-Error "bolt-win-x64.exe not found in release $version"
    exit 1
}
...
Invoke-WebRequest -Uri $exeAsset.browser_download_url -OutFile "$BIN_DIR\bolt.exe"

# Download bolt.d.ts to ~/.bolt/ (for plugin IDE support)
if ($dtsAsset) {
    Invoke-WebRequest -Uri $dtsAsset.browser_download_url -OutFile "$BOLT_HOME\bolt.d.ts"
