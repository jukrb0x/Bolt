...
test("build-program respects build_type and platform params", async () => {
  const ctx = makeCtx()
  await uePlugin.handlers["build-program"]({ target: "AnvilSmith", build_type: "debug", platform: "Win64" }, ctx)
  const cmd = ctx.logged.find(l => l.includes("Build.bat")) ?? ""
...
test("build-program throws when target is empty string", async () => {
  const ctx = makeCtx()
  await expect(uePlugin.handlers["build-program"]({ target: "" }, ctx)).rejects.toThrow("No target specified")
})
...
test("fix-dll moves 0-byte DLL files to trash dir", async () => {
  ...
  const ctx2: BoltPluginContext = { cfg: fakeCfg, dryRun: false, logger: new Logger({ sink: () => {} }) }
